...
SpriteRenderer::SpriteRenderer(Shader* shaderProg)
{
        ...
        float rotateAngle, glm::vec3 color, float fill, bool vertical)
{
    ...
    shader->setMat4("model", model);
    shader->setVec3("spriteColor", color);
    shader->setBool("useFill", true);
    shader->setBool("verticalFill", vertical);
    shader->setFloat("fillAmount", fill);
